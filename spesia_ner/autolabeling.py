...
from .datasets import ClinicalRecordsDataset, DataCollatorForMultiLabelTokenClassification
...
class AutoAnnotator:
    ...
    def _predict(self, 
                 records: transformers.tokenization_utils_base.BatchEncoding | list[transformers.tokenization_utils_base.BatchEncoding]
                 ) -> np.ndarray:
    ...
    def _map_prediction_to_tokens(self, 
        ...
        for current_record, current_record_predictions in zip(records, predictions):
            ...
            for token_index, pred in enumerate(current_record_predictions):
                text = self.tokenizer.decode(current_record.input_ids[token_index], skip_special_tokens=True)
    ...
    def annotate(self, dataset_path: Path | str) -> ClinicalRecordsDataset:
        ...
        for idx in tqdm(range(0, len(self.dataset), self.batch_size), desc="Annotating records", total=(len(self.dataset))//self.batch_size):
            ...
            for record_idx in range(idx, min(idx + self.batch_size, len(self.dataset))):
                self.dataset.records[record_idx].annotations = merged_annotations[record_idx - idx]
